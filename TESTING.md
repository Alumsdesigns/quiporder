...
Automated unit tests were written using Django's TestCase framework.
...
2. Clear cache
```bash
rm -rf equipment/tests/__pycache__
```
```bash
find . -type d -name "__pycache__" -exec rm -rf {} + 2>/dev/null
```

3. Run tests ignore migrations
```bash
coverage run --source='equipment,users' --omit='*/migrations/*' manage.py test
```
...
**Note** sometimes you need to fix and regenerate HTML Report
The HTML report can sometimes remain outdated. Run these commands if whats in the terminal mismatched with whats in the browser:
...
*Note: Coverage excludes migration files. Full test coverage is planned for future iterations.*

Terminal view:
![Coverage Report in the terminal](docs/automated_testing_coverage_report_image/quiporder-coverage-report_html_terminal_report.png)

Browser View:
![Coverage Report in the Browser generated by html](docs/automated_testing_coverage_report_image/quiporder-coverage-report_html_browser_report.png)
